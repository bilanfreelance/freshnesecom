...
let searchSelect = document.querySelector('.search-header__title');
...
for (let index = 0; index < checkboxCategories.length; index++) {
	...
	checkboxCategorie.addEventListener("change", function (e) {
		...
		if (checkboxActiveCategories.length > 0) {
			...
			let searchQuantity = searchSelect.querySelector('.search-header__quantity');
...
// Вид каталогу товарів (список чи таблиця)

let buttonGrid = document.querySelector('.top-filter__grid');
let buttonList = document.querySelector('.top-filter__list');

let categoryList = document.querySelector('.category-list');
let categoryGrid = document.querySelector('.category-grid');

let pageContentCategory = document.querySelector('.page__content_category');
if (pageContentCategory) {
	buttonGrid.addEventListener("click", function (e) {
		...
		categoryList.classList.remove('_active');
		categoryGrid.classList.add('_active');
	});
	buttonList.addEventListener("click", function (e) {
		buttonList.classList.add('_active');
		buttonGrid.classList.remove('_active');
		categoryList.classList.add('_active');
		categoryGrid.classList.remove('_active');
...
let slider_about = new Swiper('.slider-customers__body', {
	/*
	effect: 'fade',
	autoplay: {
		delay: 3000,
	...
	*/
	...
	// Arrows
	navigation: {
		nextEl: '.slider-customers__arrow_next',
		prevEl: '.slider-customers__arrow_prev',
	},
...
//Ініціалізація слайдера на мобільних екранах

const sliderDesc = document.querySelector('.description__slider');
if (sliderDesc) {
	let mySwiper;
	function mobileSlider() {
		if (window.innerWidth <= 767 && sliderDesc.dataset.mobile == 'false') {
			mySwiper = new Swiper(sliderDesc, {
				observer: true,
				observeParents: true,
				spaceBetween: 10,
				autoHeight: true,
			...
			sliderDesc.dataset.mobile = 'true';
		}
		if (window.innerWidth > 767) {
			sliderDesc.dataset.mobile = 'false';
			if (sliderDesc.classList.contains('swiper-container-initialized')) {
				mySwiper.destroy();
			}
		}
	}
